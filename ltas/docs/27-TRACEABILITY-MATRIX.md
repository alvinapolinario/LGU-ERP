...
| LTAS-FR-COMMITTEE-002 | Committees | `/measures/{id}/referrals`, `/committees/{id}/referrals`, `/committees/{id}/meetings` | committee_referrals, committee_meetings, meeting_referrals (hearings/reports later) | T-FR-COMMITTEE-002 → AC-COMMITTEE-002 | 3 | Joint referral retains each committee row; meetings bind those referrals without certified minutes; reports remain later |
...
| LTAS-FR-SESSION-001 | Sessions | `/sessions`, `/calendar` | legislative_sessions, session_agenda_items (readings/minutes later) | T-FR-SESSION-001 → AC-SESSION-001 | 4 | Secretariat-recorded sitting and agenda; published revisions and certified minutes remain later |
...
| LTAS-FR-ATTENDANCE-001 | Attendance | `/sessions/{id}/attendance` | session_attendance (events/quorum later) | T-FR-ATTENDANCE-001 → AC-ATTENDANCE-001 | 3–4 | Person disposition recorded; not a quorum snapshot; closed session freezes changes |
...
| LTAS-FR-VOTE-001 | Voting | `/sessions/{id}/votes` | session_votes (ballots/certify later) | T-FR-VOTE-001 → AC-VOTE-001 | 4–5 | Secretary-entered tally is RECORDED only; ballots and certification remain later |
...
| LTAS-FR-LIBRARY-001 | Library | `/library` | legislative_measures, documents, legislative_sessions, committee_meetings, committees | T-FR-LIBRARY-001 → AC-LIBRARY-001 | 7 (constrained) | Filters and counts respect scope; scans remain metadata-searchable. Official archive/codification later |
...
| LTAS-FR-DASHBOARD-001 | Dashboard | `/measures/stats`, `/reports` | scoped source queries, tasks, deadlines | T-FR-DASHBOARD-001 → AC-DASHBOARD-001 | 2, 8 (constrained) | KPI reconciles with identical filtered report keys `measures.proposed` and `referrals.pendingMeasures` |
| LTAS-FR-REPORT-001 | Reports | `/reports` | legislative_measures, committee_referrals, sessions, votes | T-FR-REPORT-001 → AC-REPORT-001 | 8 (constrained) | `P-REPORT-INTERIM-1` counts; D-11 official catalog later |
