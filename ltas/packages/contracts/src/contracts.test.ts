...
describe('public input contracts',()=>{
  ...
  it('scopes a task page to one case',()=>{const id='20000000-0000-4000-8000-000000000001';expect(taskQuerySchema.parse({ownerId:id,measureId:id,limit:'50'})).toMatchObject({ownerId:id,measureId:id,page:1,limit:50});expect(taskQuerySchema.safeParse({ownerId:'not-an-id'}).success).toBe(false);});
  it('bounds a calendar window',()=>{expect(calendarQuerySchema.parse({from:'2026-09-01T00:00:00.000Z',to:'2026-10-01T00:00:00.000Z',limit:'5'})).toMatchObject({page:1,limit:5});expect(calendarQuerySchema.safeParse({from:'September'}).success).toBe(false);});
